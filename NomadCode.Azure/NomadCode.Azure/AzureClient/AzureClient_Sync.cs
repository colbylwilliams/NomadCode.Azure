...
#endif
            string queryId = null;
            IMobileServiceTableQuery<T> query = null;

            try
            {
                query = where == null ? table?.CreateQuery () : table?.Where (where);

                queryId = where == null ? $"{table?.TableName ?? typeof (T).Name}" : $"{table?.TableName ?? typeof (T).Name}{syncQueryFormat}";
            ...
            catch (MobileServicePushFailedException mspfe)
            {
                ...
                switch (mspfe.PushResult.Status)
                {
                    case MobileServicePushStatus.CancelledByAuthenticationError:
                        ...
                        {
                            if (!string.IsNullOrEmpty (queryId) && query != null)
                            {
                                await table?.PullAsync (queryId, query);
                            }
                        }
